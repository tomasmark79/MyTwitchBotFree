...
char8_t *EmojiBuilder::encodeUtf8Sequence (const char32_t *emojiCodePoints,
...
EmojiBuilder::EmojiBuilder ()
{
  ...
  else
  {
    ...
    constructEmojiPropertiesMap (m_emojiPropertiesMap,
                                 static_cast<std::istream &> (ss));
  }
}
void EmojiBuilder::constructEmojiPropertiesMap (
  std::map<int, EmojiPropertiesStructure> &epm, std::istream &file)
{
  int mapKey = 0;
  ...
  while (std::getline (file, line))
  {
             ...
             && (line.find ("# group:") == std::string::npos))
    {
      ...
      char8_t *end = encodeUtf8Sequence (emojiCodePoints.data (),
...
std::ifstream EmojiBuilder::loadEmojiAssetsFromFile ()
{
  ...
  // if (!is)
  //     std::cerr << "Emoji asset test file not found. Loading hardcoded
  //     definition." << std::endl;

  return is;
}
std::string
EmojiBuilder::getEmojiStringCharByCodePoint (char32_t *emojiCodePoints,
                                             size_t length)
{
  if (m_isPopulated)
  {
...
char8_t
EmojiBuilder::getEmojiChar8_tCharByCodePoint (char32_t *emojiCodePoints,
                                              size_t length)
{
  char8_t buffer[32];
  char8_t *end = encodeUtf8Sequence (emojiCodePoints, length, buffer);
...
std::string
EmojiTransmitter::getEmojiStringCharByCodePoint (char32_t *emojiCodePoints,
                                                 size_t length)
{
  return emojiBuilder.getEmojiStringCharByCodePoint (emojiCodePoints, length);
}
char8_t
EmojiTransmitter::getEmojiChar8_tCharByCodePoint (char32_t *emojiCodePoints,
                                                  size_t length)
{
...
std::string EmojiTransmitter::getRandomEmojiFromGroup (std::string emojiGroup)
{
  if (emojiBuilder.m_isPopulated)
  {
    ...
    std::uniform_int_distribution<> dis (1,
                                         getSizeOfGroupItems (emojiGroup) - 1);
...
std::string
EmojiTransmitter::getEmojiesFromSubGroup (std::string emojiSubGroup)
{
  if (emojiBuilder.m_isPopulated)
  {
    std::string emojis = "";
...
std::vector<std::string> EmojiTransmitter::getEmojiGroupsNames ()
{
  if (emojiBuilder.m_isPopulated)
  {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap)
    {
      if (std::find (vecGroups.begin (), vecGroups.end (),
...
std::vector<std::string> EmojiTransmitter::getEmojiSubGroupsNames ()
{
  if (emojiBuilder.m_isPopulated)
  {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap)
    {
      if (std::find (vecSubGroups.begin (), vecSubGroups.end (),
                     epm.second.m_emojiSubGroup)
          == vecSubGroups.end ())
      {
        vecSubGroups.push_back (
          epm.second.m_emojiSubGroup); // SubGroups map source
      }
    }
    return vecSubGroups;
  }
...
std::string
EmojiTransmitter::getEmojiStringByIndexFromGroup (std::string emojiGroup,
                                                  int index)
{
  if (emojiBuilder.m_isPopulated)
  {
...
std::string
EmojiTransmitter::getEmojiStringByIndexFromSubGroup (std::string emojiSubGroup,
                                                     int index)
{
  if (emojiBuilder.m_isPopulated)
  {
...
void EmojiTransmitter::printEmojiGroup (std::string emojiGroup)
{
  if (emojiBuilder.m_isPopulated)
  {
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap)
    {
      if (epm.second.m_emojiGroup == emojiGroup)
      {
        getEmojiChar8_tCharByCodePoint (epm.second.m_emojiCodePoints.data (),
                                        epm.second.m_emojiCodePoints.size ());
      }
    }
  }
...
void EmojiTransmitter::printGroupsText ()
{
  if (emojiBuilder.m_isPopulated)
  {
    ...
    for (auto &epm : emojiBuilder.m_emojiPropertiesMap)
    {
      if (std::find (groups.begin (), groups.end (), epm.second.m_emojiGroup)
          == groups.end ())
      {
