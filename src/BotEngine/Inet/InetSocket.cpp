...
Socket::Socket (void)
  : mConnected (false), mInvalid (true), mReadReady (false),
    mReadyReadyOOB (false), mWriteReady (false), mTimedout (false),
    mSocket (-1), mReceivedBytes (0)
{
  // empty
}

Socket::Socket (int sock)
  : mConnected (true), mInvalid (false), mReadReady (false),
    mReadyReadyOOB (false), mWriteReady (false), mTimedout (false),
    mSocket (sock), mReceivedBytes (0)
{
  // empty
}
...
int Socket::bind (const char *port, const char *ip)
{
  ...
  for (addrinfo *p = res; p != NULL; p = p->ai_next)
  {
    ...
    if (::setsockopt (mSocket, SOL_SOCKET, SO_REUSEADDR, &socketReuseValue,
                      sizeof (socketReuseValue))
        == -1)
    {
      cerr << "Unable to reuse socket " << errno << " " << strerror (errno)
           << endl;
      ::close (mSocket);
      mSocket = -1;
      continue;
...
Socket *Socket::accept (bool block)
{
  ...
  if (selectVal < 0)
  {
    std::cerr << "Error encountered in ::select " << errno << ", "
...
bool Socket::connected (void) const { return mConnected; }

int Socket::recvLength (void) const { return mReceivedBytes; }
...
int Socket::connect (const char *ip, const char *port)
{
  ...
  for (addrinfo *addr = results; addr != NULL; addr = addr->ai_next)
  {
    ...
    if (sock == -1)
    {
      cout << "Unable to connect socket! " << errno << strerror (errno)
           << endl;
      continue;
    }
...
int Socket::receive (char *buffer, int bufferLength, int timeout)
{
  ...
  do
  {
    ...
    if (mReadReady || mReadyReadyOOB)
    {
      ...
      int ret
        = ::recv (mSocket, buffer + received, bufferLength - received, flags);
...
void Socket::checkForReady (short events, int timeout)
{
  ...
  if (ret == -1)
  {
    cerr << "Error encountered: " << ret << ", errno: " << errno
         << strerror (errno) << endl;
    mConnected = false;
  }
