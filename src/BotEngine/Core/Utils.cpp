...
std::string join (const std::vector<std::string>& parts, const std::string& delimiter) {
  std::string ret = "";
  for (std::vector<std::string>::const_iterator iter = parts.begin (); iter != parts.end ();
       iter++) {
    ret += *iter;
    auto tmpIter = iter;
    tmpIter++;
...
CommandParts parseCommand (const Message& incoming) {
  ...
  if (parts.at (0).at (0) == ':') {
    // We have a prefix, meaning that the second item (if it exists) is the
    // command.
    prefix = parts.at (0);
    parts.erase (parts.begin ());
  }
  ...
  return { true, command, state, prefix, params };
}
