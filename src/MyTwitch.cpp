...
namespace library
{
    ...
    : m_assetsPath (assetsPath)
  {
    ...
    auto pointAdderLambda
      = [] (Address &pointAdder, Address &commandParser) -> void
    {
      ...
      while (command != "SHUTDOWN")
      {
        ...
        if (PostOffice::isValidInstance (postOffice)
            && postOffice->checkMail (myAddress))
        {
    ...
    auto commandParserLambda
      = [] (CommandSet &mCommands, Address &commandParser, Address &pointAdder,
            Address &transceiver) -> void
    {
      PostOffice *postOffice = PostOffice::instance ();
      ...
      while (command != "SHUTDOWN")
      {
        ...
        cout << "State='" << state << "' Prefix='" << prefix << "' Command='"
    ...
    auto transceiverLambda
      = [] (Address &commandParser, Address &transceiver, Address &pointAdder,
            Address &apiAddress) -> void
    {
      Socket twitchConnection;
      int connectionRet
        = twitchConnection.connect ("irc.chat.twitch.tv", "6667");
      ...
      while (twitchConnection.connected ())
      {
            ...
            && throttle.check (messagesPerTick))
        {
          ...
          {
            int sent
              = twitchConnection.send (ptr + sentBytes, size - sentBytes, 100);
        ...
        if (messagesPerTick == 20) // The base number.
        {
          ...
          Message response
            = msg.sendAndAwaitResponse (commandParser, pointAdder);
          if (response.size () == 0)
          {
            continue;
          }
        ...
        int receivedBytes
          = twitchConnection.receive (buffer, bufferLength - 1);
        if (receivedBytes == 0)
        {
          usleep (30);
          continue;
        ...
        do
        {
          ...
          {
            incompleteMessage
              = incompleteMessage.substr (0, incompleteMessage.size () - 1);
    ...
    auto apiServerLambda
      = [] (Address &transceiver, Address &apiAddress) -> void
    {
            ...
            [this] (Address commandParser, Address transceiver) -> void
            {
              while (!stopTimerThread.load ())
              {
                ...
                std::this_thread::sleep_for (
                  std::chrono::seconds (EMOJI_INTERVAL_SEC));
              }
            },
            commandParser,
    ...
    mCommands.registerCommand (
      { "!points", "return the number of points you have accrued.",
             ...
             const std::vector<std::string> &params) -> void
        {
          ...
          Message response
            = pointInfo.sendAndAwaitResponse (commandParser, pointAdder);
          if (response.size () == 0)
          {
          ...
          cout << "Retrieving points for '" << user << "' on " << channel
               << endl;
          if (pointValue == "0")
          {
            Message msg ("PRIVMSG " + channel + " :@" + user
          ...
          {
            Message msg ("PRIVMSG " + channel + " :@" + user
                         + ", your point total is " + pointValue + ".\r\n");
            msg.send (commandParser, transceiver);
          }
        } });
    ...
    mCommands.registerCommand (
      { "!startRaffle", "start a point raffle.",
             ...
             const std::vector<std::string> &params) -> void
        {
          ...
          std::vector<std::string> parms = cmdParts.params;
          std::string role = parms.at (0);
          cout << "=== Prefix='" << prefix << "' Command='" << command
               << "' Params=['" << join (parms, "', '") << "'], Role='" << role
               << "'" << endl;
    ...
    threads.push_back (std::thread (
      transceiverLambda, std::ref (commandParser), std::ref (transceiver),
      std::ref (pointAdder), std::ref (apiAddress)));

    threads.push_back (std::thread (
      commandParserLambda, std::ref (mCommands), std::ref (commandParser),
      std::ref (pointAdder), std::ref (transceiver)));

    threads.push_back (std::thread (pointAdderLambda, std::ref (pointAdder),
                                    std::ref (commandParser)));

    threads.push_back (std::thread (apiServerLambda, std::ref (transceiver),
                                    std::ref (apiAddress)));
  ...
  MyTwitch::~MyTwitch () { LOG_DEBUG ("MyTwitch deconstructed."); }
